...
const getTeslaData = async (statePymtObj, metaVehicleObj, setTeslaModels) => {
  ...
  if (metaVehicles.length > 0) {
    for (let i = 0; i < metaVehicles.length; i++) {
      ...
      let parsedValue = await parseAllStringValues(metaVehicles[i]);
      ...
      if (statePymtObj !== undefined) {
        ...
        let paymentObject = await populatePaymentObject(purchasePrice, statePymtObj);
  ...
  return vehicleObj
  // await setTeslaModels(vehicleObj);
};
