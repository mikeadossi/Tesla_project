        ...
        <div className="powerwallFinance_container"> 
            ...
            <div className="app_Solar_cost_div">
                ...
                <span className="app_inline-block app_Solar_cost">
                    ${Math.ceil(loan_pymts(powerwallPricing["select_2PW"]["cash_price"])[0]).toLocaleString("en-US")}/mo
                </span>
            </div>
            <div className="app_Solar_cost_div">
                <span className="app_inline-block">Loan After Est. Incentives</span>
                <span className="app_inline-block app_Solar_cost">
                    ${Math.ceil(loan_pymts(powerwallPricing["select_2PW"]["cash_price"])[1]).toLocaleString("en-US")}/mo
                </span>
            </div>
            <div className="solar_small_print">
              ${Math.ceil(loan_pymts(powerwallPricing["select_2PW"]["cash_price"])[2]).toLocaleString("en-US")} down, 120 months, 2.99% APR
          </div>
        </div> 
    );
}
