...
const DisplayApplyAllWarning = ({
  ...
  const spacedVehicleName =
    teslaModels["vehicle_render"][currentModelName]["model"]; // Model 3
  ...
  const permitToShowToggle = () => {
    ...
    if (!currentUser) {
      if (doNotSetCookie && doNotSetCookie === "true") {
        ...
        (applyAllCookie && applyAllCookie === "true") ||
        applyAllCookie === "false"
      ) {
        setShowToggle("true");
        return;
      }
  ...
  return (
    <div className="displayApplyAllWarning_super" style={bkgClr} >
